...
from app.models.category import manager, managerdb
...
async def insedrt(
    manager: manager.managerInsmodel = Body(..., embed=True),
    ...
    # )
    try:
        ct = jsonable_encoder(manager)
        resp = managerdb.insert_doc("", json=ct)
        # dict_cache.runApp()

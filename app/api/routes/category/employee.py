...
from app.models.category import employee, employeedb
...
async def insedrt(
    employee: employee.employeeInsmodel = Body(..., embed=True),
    ...
    # )
    try:
        ct = jsonable_encoder(employee)
        resp = employeedb.insert_doc("", json=ct)
        # dict_cache.runApp()

...
from app.models.category import userInformation, userInformationdb
...
async def insedrt(
    userInformation: userInformation.userInformationInsmodel = Body(..., embed=True),
    ...
    # )
    try:
        ct = jsonable_encoder(userInformation)
        resp = userInformationdb.insert_doc("", json=ct)
        # dict_cache.runApp()
...
async def get():
    try:
        resp = list(userInformationdb.getModel().find())
